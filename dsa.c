...
int main(int argc, char* argv[]) {
    ...
    printf("Calling Move Pages\n");
    TIME_START
    call_move_pages(mem_u, arr_size, 1);
    TIME_STOP
...
void call_move_pages(Page *mem, long long num_pages, int new_device) {
    ...
    pages  = (void **)malloc(size);
    memset(pages, 0, size);
    for (long long i = 0; i < num_pages; i++) {
        pages[i] = (void *)(mem + i);
    }

    nodes  = (int *)malloc(size);
    memset((void *) nodes, 0, size);
    for (long long i = 0; i < num_pages; i++) {
        nodes[i] = new_device;
    }

    status = (int *)malloc(size);
    memset(status, 0, size);

    move_pages(0, num_pages, pages, nodes, status, MPOL_MF_MOVE);
...
int dsa_move(Page *src, Page *dest, long long buffer_size, long long arr_size) {
/*     dml_path_t execution_path = DML_PATH_SW; //software path */
    dml_path_t execution_path = DML_PATH_HW; //hardware path
