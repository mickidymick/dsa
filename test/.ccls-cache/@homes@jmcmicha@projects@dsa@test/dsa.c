...
/*     int retry = 0; */
/*     while(status == DML_STATUS_WORK_QUEUES_NOT_AVAILABLE && retry < 1000) { */
/*         status = dml_submit_job(dml_job_ptr); */
/*         retry++; */
/*     } */
...
#ifdef DSA_BATCH
    uint64_t num_batches  = buffer_size / MAX_BATCH_SIZE;
    if (num_batches)
    uint64_t remainder    = buffer_size % MAX_BATCH_SIZE;
...
int dsa_threaded_async_copy(void *dest, void *src, uint64_t buffer_size) {
    ...
    num_threads = buffer_size / VMEM_PAGE_SIZE;
    if (num_threads < MAX_JOBS) {
        threads = num_threads;
    }
    size      = buffer_size / threads;
    remainder = (buffer_size) % threads;



/*     printf("%lu %lu %f %d\n", size, remainder, num_threads, threads); */
/*     return 0; */
/*     uint64_t size = buffer_size / threads; */
    uint64_t s = buffer_size / VMEM_PAGE_SIZE / threads;
/*     printf("move per thread: %lu\n", s); */
    ...
    for (int i = 0; i < threads; i++) {
        ...
        pthread_create(&th_arr[i], &attr, dsa_threaded_a_copy, a_arr[i]);
    }
    for (int i = 0; i < threads; i++) {
        pthread_join(th_arr[i], NULL);
    }
